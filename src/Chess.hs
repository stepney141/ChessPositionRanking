...
import Chess.Internal.Game qualified as G
import Chess.Internal.Move
import Chess.Internal.Notation qualified as N
...
move ::
    ...
    String ->
    Maybe GameState
move game moveStr = do
    m <- N.parseMove game moveStr
    applyMove game m
...
pieceAt ::
    ...
    String ->
    Maybe Piece
pieceAt b coordinateStr = do
    coords <- parseCoordinate coordinateStr
    getPiece b coords
