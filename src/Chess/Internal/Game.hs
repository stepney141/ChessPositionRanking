...
applyMove game move = case moveError of
  ...
  where
    moveError = isMoveError game move
    newGame =
        State
...
isInsufficientMaterialWithBishops _ white black | not (onlyBishops white && onlyBishops black) = False
  where
    onlyBishops pieces = not (any (/= Bishop) pieces)
isInsufficientMaterialWithBishops board _ _ = bishopsOnWhite /= bishopsOnBlack
  where
    whiteSquaresWithBishops = filter (\x -> getSquareColor x == White) $ getSquaresWithPieces board Bishop
...
getWinner game
    | isCheckmate game = Just $ opponent $ currentPlayer game
    | otherwise = Nothing
