...
data Move
    = Movement Piece Coordinates Coordinates
...
generateAllMoves game = filter isLegalMove $ generateAllPotentialMoves game
  where
    isLegalMove move = isNothing (isMoveError game move)
...
isMoveError game move
    | not (isCorrectPlayer game move) = Just WrongPlayer
...
isCorrectStartPiece board (Piece color pieceType) coordinates =
  ...
  where
    boardPiece = getPiece board coordinates
...
areCoordinatesValid start end
    | start == end = Just InvalidCoordinates
...
generateAllKnightMoves game coords =
    map (Movement piece coords) emptySquares
        ++ map (Capture piece coords) opponentSquares
  where
    squares = filter isInsideBoard [sumSquares coords jump | jump <- knightPattern]
    board = stateBoard game
...
generateAllKingMoves game coords =
    map (Movement piece coords) moveSquares
...
generateAllPawnMoves game coords@(row, _) = move ++ doubleMove ++ captures ++ promotions ++ enpassant
  where
    ...
    promotionCapture square =
        if isOpponentSquare board square player
            then map (Promotion (Piece player Pawn) coords square) [Rook, Bishop, Knight, Queen]
            else []
    promotionMove square =
        if isEmpty board square
            then map (Promotion (Piece player Pawn) coords square) [Rook, Bishop, Knight, Queen]
            else []
    promotions =
        if isNextToPromotionRow
...
kingMoveSquares game start = emptySquares
  where
    allSquares = map (sumSquares start) queenPattern
    board = stateBoard game
...
isCastlingPossible game player castling = castling `elem` possibleCastlings player && all (isEmpty board) squares && traverseSquaresNotThreatened
  where
    squares = castlingSquares player castling
    board = stateBoard game
...
movementsInDirection game start direction = map (Movement piece start) squares
  where
    piece = fromJust $ getPiece (stateBoard game) start
...
iterateMovementSquares = iterateDirection isEmptySquare
  where
    isEmptySquare game' = isEmpty (stateBoard game')
...
iterateCaptureSquares game start direction = case squaresNotEmpty of
  ...
  where
    squares = iterateDirectionInsideBoard start direction
    board = stateBoard game
    (Piece player _) = fromJust $ getPiece board start
