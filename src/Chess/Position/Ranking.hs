...
data URPosition = URPosition
    ...
    , wEP :: Square
    , bEP :: Square -- set in enPassantRanking
    , wEPOpp :: Int
    , bEPOpp :: Int -- set in epOppRanking
...
toPosition pos = Position{diagram = diag, sideToMove = stm, castlings = cM, enPassant = ep, num0 = n0, num1 = n1}
  where
...
urChoices (URPosition{whiteToMove = wtm, wEP = wep, bEP = bep, minOpp = minopp}) diag = listRanking choices
  where
    ...
    eps =
        if wtm
            then
                [(pep - 1, pep) | epfile > 0, diag ! (pep - 1) == 'P']
                    ++ [(pep + 1, pep) | epfile < 7, diag ! (pep + 1) == 'P']
            else
                [(pep, pep - 1) | epfile > 0, diag ! (pep - 1) == 'p']
...
fromPosition (Position{diagram = diag, sideToMove = stm, castlings = cM, enPassant = ep, num0 = n0, num1 = n1}) =
    let
        wtm = stm == 'w'
        ...
        pos =
            URPosition
                ...
                , pieceCnts = pccnts
                , wEP = pep
                , bEP = pep -- only partially correct
                , wEPOpp = 0
                , bEPOpp = 0
                , oppSep = 0 -- not set
                , oppList = []
     ...
     in
        pos'{pawnString = map haspawn pwnSquares, pieceString = map (diag !) pcSquares}

-- diagram with only fixed (castling and en-passant) placements
fixDiagram :: URPosition -> Diagram
fixDiagram pos = emptyDiagram // kingAssocs // rookAssocs // epAssocs
  where
    ...
    epAssocs = if ep == 0 then [] else [(wep, 'P'), (bep, 'p'), (c, enPassantSymbol), (c + 8, enPassantSymbol)]
      where
        c = if wtm then bep - 16 else wep + 8

-- with additional comma separated opposing pawns
oppDiagram :: URPosition -> Diagram
oppDiagram pos@(URPosition{oppList = oL}) = foldl opp (fixDiagram pos) oL
  where
...
pawnDiagram pos@(URPosition{pawnString = pS}) = oppDiag // zip pwnSquares pS
  where
    oppDiag = oppDiagram pos
    pwnSquares = filter (\c -> oppDiag ! c == emptySymbol) pawnSquares
...
sideToMoveRanking pos = Ranking size cmp unrank rank
  where
    size = 2
    cmp = compare `on` rank
...
caseRanking pos = Ranking size cmp unrank rank
  where
...
armyStats_ = array ((0, 0), (2, 1)) [((fr, fp), collectedArmies fr fp) | fr <- [0 .. 2], fp <- [0 .. 1]]
  where
    -- group elements in list paired with common key
    collectedArmies fr fp = M.fromListWith (++) . map (\army@(Army _ _ pcs) -> (armyStat army, [pcs])) $ armies fr fp
      where
...
wArmyStatRanking pos@(URPosition{fixwr = fwr, fixp = ep}) = Ranking size cmp unrank rank
  where
...
bArmyStatRanking pos@(URPosition{fixbr = fbr, fixp = ep}) = Ranking size cmp unrank rank
  where
...
guardRanking pos = Ranking size cmp unrank rank
  where
...
wArmyRanking pos = Ranking size cmp unrank rank
  where
...
bArmyRanking pos = Ranking size cmp unrank rank
  where
...
castleRanking pos = Ranking size cmp unrank rank
  where
    ...
    unrank i' = pos{castleMap = cM}
      where
        ...
        fCM 1 i = 1 + i
        fCM 2 _ = 3
    rank (URPosition{castleMap = cM}) = fromIntegral $ wi * mbr + bi
      where
...
enPassantRanking pos = Ranking size cmp unrank rank
  where
    ...
    multEP 1 = 14
    cmp = compare `on` rank
    unrank i' = if ep == 0 then pos else pos{wEP = wep, bEP = bep}
      where
        i = fromIntegral i'
        i7 = i `div` 2
        i2 = i `mod` 2
    ...
    rank (URPosition{wEP = wep, bEP = bep}) = if ep == 0 then 0 else fromIntegral $ i7 * 2 + i2
      where
        wfile = wep `mod` 8
        bfile = bep `mod` 8
...
epOppRanking pos@(URPosition{fixp = ep, wArmyStat = ArmyStat _ wp _ _, bArmyStat = ArmyStat _ bp _ _, minOpp = mo}) = Ranking size cmp unrank rank
  where
...
sandwichRanking pos@(URPosition{minOpp = mo}) = Ranking size cmp unrank rank
  where
...
opposeRanking pos = Ranking sz cmp unrnk rnk
  where
    ...
    cmp = compare `on` rnk
    epR = sizeRanking $ epRCase wopp bopp
      where
    ...
    unrnk i = pos{oppList = (if ep == 0 then placeOpp [0 .. 7] p s else placeEPOpp) i}
      where
        placeEPOpp i = merge epOpps (placeOpp fs p' s' i')
          where
        ...
        placeOpp [] 0 _ 0 = []
        placeOpp (f : fs) p s i = if i < size0 then placeOpp fs p s i else setOpp (fileOpps f s) (i - size0)
          where
            size0 = fromIntegral $ mopps p s n
            n = length fs
            setOpp (opp@(b, w) : opps) i =
                if i < size'
                    then opp : placeOpp fs (p - 1) s' i
                    else setOpp opps (i - size')
              where
                size' = fromIntegral $ mopps (p - 1) s' n
                s' = s - ((w - 8 - b) `div` 8)
    rnk (URPosition{oppList = oL}) = if ep == 0 then unplaceOpp [0 .. 7] s oL else unplaceEPOpp
      where
        unplaceEPOpp = rank epR (epOpps, unplaceOpp fs s' opps)
          where
            (epOpps, opps) = partition (\(b, w) -> b == bep || w == wep) oL
            fs = [0 .. 7] \\ [wep `mod` 8, bep `mod` 8]
            s' = s - sum [(w - 8 - b) `div` 8 | (b, w) <- epOpps]
        unplaceOpp _ 0 [] = 0
        unplaceOpp (f : fs) s os@(opp@(b, w) : opps') = if b `mod` 8 /= f then unplaceOpp fs s os else sizeNo + rnkOpp (fileOpps f s)
          where
            sizeNo = fromIntegral $ mopps p s n
            p = length os
            n = length fs
            rnkOpp (fopp@(fb, fw) : fopps) =
                if fopp == opp
                    then unplaceOpp fs s' opps'
                    else size' + rnkOpp fopps
              where
                size' = fromIntegral $ mopps (p - 1) s' n
                s' = s - ((fw - 8 - fb) `div` 8)
...
pawnRanking pos = Ranking size cmp unrnk rnk
  where
...
pieceRanking pos = Ranking size cmp unrnk rnk
  where
