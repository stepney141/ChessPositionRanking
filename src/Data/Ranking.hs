...
multinomialRanking itemCounts = Ranking size cmp unrank rank
  where
    cnts = map snd itemCounts
    total = sum cnts
    size = fac total `div` product (map fac cnts)
      where
        fac n = fac_ ! n where
        fac_ = listArray (0, total) (scanl (*) 1 [1 .. fromIntegral total])
    cmp [] [] = EQ
    cmp (a : as) (b : bs) = if cmpiab == EQ then cmp as bs else cmpiab
      where
        cmpiab = compare ia ib
        (Just ia) = elemIndex a items
        (Just ib) = elemIndex b items
        items = map fst itemCounts
    unrank = place total itemCounts size
      where
        place 0 _ _ 0 = []
        place 0 _ _ _ = error "nonzero i"
        place n itemCounts size i = setItem id itemCounts i
          where
            setItem pref (ic@(item, cnt) : ics) i =
                if i < size'
                    then item : place (n - 1) (pref ((item, cnt - 1) : ics)) size' i
                    else setItem (pref . (ic :)) ics (i - size')
              where
                size' = size * (fromIntegral cnt) `div` (fromIntegral n)
    rank = unplace total itemCounts size
      where
        unplace :: (Eq a) => Int -> [(a, Int)] -> Integer -> [a] -> Integer
        unplace 0 _ _ l = if null l then 0 else error "non-null list"
        unplace n itemCounts size (item : items) = rnkItem id itemCounts
          where
            rnkItem pref (ic@(it, cnt) : ics) =
                if it == item
                    then unplace (n - 1) (pref ((item, cnt - 1) : ics)) size' items
                    else size' + rnkItem (pref . (ic :)) ics
              where
                size' = size * (fromIntegral cnt) `div` (fromIntegral n)
...
listRanking l = Ranking size cmp unrank rank
  where
    size = fromIntegral $ length l
    cmp a b = compare ia ib
      where
...
sizeRanking itemSizes = Ranking size cmp unrank rank
  where
    size = sum . map snd $ itemSizes
    cmp (a0, i0) (a1, i1) = if cmpai == EQ then compare i0 i1 else cmpai
      where
        cmpai = compare ai0 ai1
        (Just ai0) = elemIndex a0 items
        (Just ai1) = elemIndex a1 items
        items = map fst itemSizes
    unrank = exhaust itemSizes
      where
...
mapR inv f ra = Ranking sz cmpr unrnk rnk
  where
    sz = size ra
...
bindR inv ra arb = Ranking sz cmpr unrnk rnk
  where
    sizeR = sizeRanking [(i, size . arb . unrank ra $ i) | i <- [0 .. size ra - 1]]
    sz = size sizeR
    cmpr b0 b1 = if cmp ra a0 a1 == EQ then cmp (arb a0) b0 b1 else cmp ra a0 a1
      where
        (a0, a1) = (inv b0, inv b1)
...
bindUR inv ra arb = Ranking sz cmpr unrnk rnk
  where
    sz = sizeRa * sizeRb
    sizeRa = size ra
