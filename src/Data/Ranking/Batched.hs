...
import Data.List
import Data.Ranking qualified as R
...
sizeRanking itemSizes = BRanking size cmp unrank rank
  where
...
bindR inv ra arb = BRanking sz cmpr unrnk rnk
  where
    aranks = [0 .. size ra - 1]
    ...
    cmpr b0 b1 = if cmp ra a0 a1 == EQ then cmp (arb a0) b0 b1 else cmp ra a0 a1
      where
        (a0, a1) = (inv b0, inv b1)
    unrnk is = concat . zipWith unrank rbs . map snd $ aibis
      where
        rbs = map arb . unrank ra . map fst $ aibis
        aibis = groupByFst . unrank sizeR $ is
    rnk bs = rank sizeR [(ai, bi) | (ai, bs) <- aibs, let rb = arb . inv . head $ bs, bi <- rank rb bs]
      where
...
bindUR inv ra arb = BRanking sz cmpr unrnk rnk
  where
    sz = sizeRa * sizeRb
    sizeRa = size ra
    ...
    rnk bs = [(ai * sizeRb + bi) | (ai, bs) <- aibs, let rb = arb . inv . head $ bs, bi <- rank rb bs]
      where
        ais = rank ra . map inv $ bs
        -- aibs = if nondecreasing ais then groupByFst (zip ais bs) else error ("decreasing " ++ show ais)
        aibs = groupByFst (zip ais bs)
