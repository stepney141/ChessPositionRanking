...
darkSquares :: [Square]
darkSquares = filter isDark boardSquares
  where
...
spareProms diag =
    let
        ...
        (pwb, pbb) = (bishopProms wb (darkcnts M.! 'B'), bishopProms bb (darkcnts M.! 'b'))
          where
...
checks diag king =
    let
     ...
     in
        [x | (x@(sq, _) : _) <- map ray dP, sq == toOpp 'p']
...
research (Position{diagram = diag, sideToMove = stm, castlings = c, enPassant = ep}) =
    let
        [wk, bk] = map (\sq -> fst . fromJust . L.find (\(i, x) -> x == sq) . assocs $ diag) "Kk"
        wtm = stm == 'w'
        ...
        checkingpcs = L.sort . map (toLower . fst) $ checksOnTM
        ...
        promRowLegal = case length promRowCheckers of
            ...
            1 ->
                (rightAngle && taxiCab > 2 && kDistToPromRow <= 1 && slack > 0)
                    || (sharpAngle && (discoverable || kDistToPromRow == 1))
            2 -> sharpAngle && (discoverable || (kDistToPromRow == 2 && slack > 0))
        doubleCheck =
            if checkingpcs == "nn"
                then "Illegal Double Knight Check"
                else
                    if promRowLegal
                        then ("Discovered Double Check" ++ stats)
                        else "Illegal Double Check"
     in
        if adjacentKings
            then "Illegal Adjacent Kings"
            else
                if tmInCheck && ntmInCheck
                    then "Illegal Both Kings in Check"
                    else
                        if ntmInCheck
                            then "Illegal Side not to move in Check"
                            else
                                if length checkingpcs > 2
                                    then "Illegal Triple Check"
                                    else
                                        if slack < 0
                                            then ("Illegal Bishops Too Monochromatic" ++ stats)
                                            else
                                                if length checkingpcs == 2
                                                    then doubleCheck
                                                    else (if length checkingpcs == 1 then "Single Check" else "No Checks") ++ stats
